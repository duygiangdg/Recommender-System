...
class MF(object):
    ...
    def normalize_Y(self):
        ...
        for n in range(n_objects):
            ...
            # take mean
            m = np.mean(ratings) if len(ratings) > 0 else 0
            self.mu[n] = m
...
ratings_base = pd.read_csv(r'data\lastfm-360k-splitted\train.csv', sep='\t', header=None)
ratings_base = ratings_base.drop(ratings_base.columns[1], axis=1)
ratings_base.columns = ['user', 'artist', 'plays']

# Drop NaN columns
ratings_base = ratings_base.dropna()

ratings_test = pd.read_csv(r'data\lastfm-360k-splitted\test.csv', sep='\t', header=None)
ratings_test = ratings_test.drop(ratings_test.columns[1], axis=1)
ratings_test.columns = ['user', 'artist', 'plays']
...
ratings_base = pd.read_csv('ml-1m/ratings.dat', sep='::', names=r_cols, encoding='latin-1', engine='python')
